...
var QueryObserver = class extends require_subscribable.Subscribable {
	...
	#shouldScheduleTimer(timeout) {
		return !require_environmentManager.environmentManager.isServer() && require_utils.resolveQueryBoolean(this.options.enabled, this.#currentQuery) !== false && require_utils.isValidTimeout(timeout);
	}
	#updateStaleTimeout() {
		this.#clearStaleTimeout();
